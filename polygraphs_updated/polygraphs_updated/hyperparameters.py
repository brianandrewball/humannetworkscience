...
class HyperParameters:
    ...
    @classmethod
    def fromYAML(cls, filename, dest=None):
        """
        Reads hyper-parameters from a .yaml file.
        """
        print(f"Loading configuration from {filename}")  # Confirm method entry
        with open(filename, "r") as fstream:
            ...
            # Unflatten data to handle nested configurations
            data = HyperParameters.unflatten(data, separator=".")
            
        
            # Check for the 'interval' parameter to handle temporal networks
            interval = data.get('simulation', {}).get('interval', None)
            print(f"Interval found: {interval}")  # Explicit check for interval
            if interval:
                print(f"Loaded interval for temporal subgraph creation: {interval}")
            else:
                print("No interval specified; assuming static network behavior.")
    
        if not dest:
            dest = cls()
    
        params = cls._merge(dest, data)  # Ensure to merge data into the destination object
        params.interval = interval  # Explicitly attach interval to the object
        print(f"Final interval parameter: {params.interval}")  # Final check for interval
    ...
    def _update(self, name, value):
        ...
        if not hasattr(self, name):
            # Dynamically add the attribute if it is not predefined
            self.add(**{name: value})
        elif not self._isvalid(value):
    ...
    def _merge(cls, dst, src):
        if isinstance(dst, HyperParameters):
            ...
            if isinstance(src, dict):
                for key, value in src.items():
                    ...
                    dst_value = dst.ht.get(key, None)  # Adjusted to handle missing keys gracefully
                    dst._update(key, cls._merge(dst_value, value))
                return dst
        ...
        if isinstance(dst, dict):
            ...
            if isinstance(src, dict):
                for key, value in src.items():
                    dst[key] = cls._merge(dst.get(key, None), value)
                return dst
        return src
...
class SimulationHyperParameters(HyperParameters):
    ...
    def __init__(self):
        ...
        self.add(repeats=1)
        self.add(steps=0)
        self.add(interval=None)  # Adding interval to handle temporal networks
...
class PolyGraphHyperParameters(HyperParameters):
    ...
    def __init__(self):
        ...
        # Parameters related to belief initialization
